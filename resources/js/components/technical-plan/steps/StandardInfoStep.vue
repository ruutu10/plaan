...
<template>
    <section class="animate-[r10fade_0.38s_ease]">
        <StepHeader eyebrow="Samm 2 / 7 · Tehniku standardinfo" title="Standardinfo"
        ...
        <div class="rounded-[22px] bg-r10-navy px-8 py-[30px] text-white">
            <ul class="m-0 flex list-none flex-col gap-4 p-0">
                <li v-for="(point, index) in points" :key="index" class="flex items-start gap-3.5">
                    <span class="mt-1.5 h-2.5 w-2.5 shrink-0 rotate-45 rounded-[1px] bg-r10-orange" />
                    <span class="text-[15px] leading-relaxed text-r10-navy-100">{{ point }}</span>
                </li>
            </ul>
        </div>
        ...
        <div class="mt-[30px] border-t border-r10-grey-200 pt-6">
            <div class="mb-1 font-r10-display text-base font-semibold tracking-[0.03em] text-r10-ink uppercase"> Ruumi
                tehniline võimekus </div>
            <p class="mt-0 mb-[18px] text-r10-grey-500">Tehnilise raideri dokumendist saad lisainfot esinemisruumis
