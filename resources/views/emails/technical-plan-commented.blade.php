{{--
    Word to the other side of a plan's conversation. Carries the remark itself
    — the letter has to be worth reading on its own, or every comment costs the
    recipient a trip to the page — and a link back to the plan, where it is
    answered.
--}}
@php
...
@section('title', 'Uus kommentaar tehnikaplaanile')

@section('preheader', $formatName.' · '.$authorName.' kommenteeris tehnikaplaani.')
...
@section('content')
    ...
    <p style="margin:0 0 24px 0; font-size:16px; line-height:1.6;">
        @if ($fromTechnicalTeam)
            Tehnikatiim lisas Sinu tehnikaplaanile kommentaari. Vastata saad plaani lehel.
        @else
            Tehnikaplaanile lisati kommentaar. Vastata saad plaani lehel.
        @endif
    </p>

    <div style="{{ $sectionTitle }}">{{ $authorName }} kirjutas</div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f2f4f7; border-radius:14px;">
        <tr>
            <td style="padding:18px 22px; font-size:15px; line-height:1.6; color:#3d4557; white-space:pre-wrap;">{{ $body }}</td>
        </tr>
    </table>
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
        ...
        @endif
        <tr>
            <td style="{{ $labelCell }}">Kommenteeris</td>
            <td style="{{ $cell }}">{{ $authorName }}@if ($fromTechnicalTeam) (tehnikatiim)@endif</td>
        </tr>
    </table>
