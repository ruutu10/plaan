...
/**
 * A show may be explained by more than one card.
 *
 * The link table was built on the assumption that a record has one reading
 * behind it, which holds for a performance and does not hold for a show: an
 ...
 * reading meant the show could only ever show its first, from an evening months
...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $this->explainShowsByTheirPerformances();
    }

    /**
     * Give the shows already imported the readings they could not hold before.
     *
     * A reading that explains a performance explains the show it was put on, so
     * the links are there to be derived — and without this, a show would only
     * catch up the next time a card happened to add a night to it.
     */
    private function explainShowsByTheirPerformances(): void
    {
        $rows = DB::table('claude_reasoning_log_subjects as links')
            ...
            ->select('links.claude_reasoning_log_id', 'performances.show_id')
            ->distinct()
            ->get();
        ...
        foreach ($rows as $row) {
            DB::table('claude_reasoning_log_subjects')->insertOrIgnore([
                ...
                'subject_type' => Show::class,
                'subject_id' => $row->show_id,
                'created_at' => $now,
                'updated_at' => $now,
            ]);
        }
